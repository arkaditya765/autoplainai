...
To support operational monitoring and capacity planning, AutoPlan AI includes a lightweight, real-time performance diagnostics and telemetry logging system.

### 9.1 Architecture: State-Embedded Logging

Rather than utilizing external logging backends or global singletons (which pose consistency and dependency conflicts during Streamlit hot-reloads), diagnostics are treated as a **first-class citizen of the application state**. 

Diagnostics data flows directly through the LangGraph `AgentState` via the `diagnostics` dictionary:

```mermaid
flowchart TD
    A["Query Start"] --> B["Clear Client Call Logs"]
    B --> C["Start Wall-Clock Timer"]
    C --> D["semantic_retrieval (Tool & Skill RAG)"]
    D --> E["LangGraph Agent Node Execution"]
    E --> F["Node Timing Decorator (timed_node)"]
    F --> G["ReAct Loop LLM & Tool Timing"]
    G --> H["Query End: Compute Total Elapsed Time"]
    H --> I["result_state['diagnostics']"]
    I --> J["Streamlit Expander Rendering"]
```

### 9.2 Metrics Captured

The telemetry collector gathers five primary classes of performance metrics per query:

1. **Total Query Execution Time**: Wall-clock duration spanning RAG retrieval, graph classification, task planning, ReAct tool execution, strategy formulation, and guardrail validation.
2. **Node Execution Waterfall**: Step-by-step performance profiles of every LangGraph node (e.g. `router_node`, `planner_node`, `orchestrator_node`, `strategy_node`, `validator_node`), showing which nodes consumed the most processing time.
3. **LLM API Calls**: Tracks the caller component, target model (e.g. `gemini-2.5-flash`), wall-clock duration of the HTTP round-trip, and specific prompt purpose.
4. **Tool Execution Timings**: Captures execution times, caller task IDs, and completion statuses (`SUCCESS`/`ERROR`) for all registered tools dynamically invoked in the ReAct loop.
5. **Embedding Calls**: Logs calls to `text-embedding-004` (e.g., during query tool/skill retrieval) with their corresponding query context and latency.

### 9.3 UI Component: Diagnostics Panel

The Streamlit interface renders these metrics in an interactive dashboard located in the **Framework Execution Trace** column:

* **Summary KPI Cards**: Real-time display of total latency, LLM invocation counts, tool executions, and embedding calls.
* **Waterfall Latency Chart**: A proportional horizontal bar chart mapping node durations dynamically.
* **Detailed Tables**: Expandable, tabular views of individual API calls and tool execution contexts for granular performance analysis.
